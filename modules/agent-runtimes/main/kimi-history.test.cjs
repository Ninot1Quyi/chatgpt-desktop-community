...
async function fixture() {
  const configDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "community-kimi-history-"));
...
test("ignores session index entries that escape the sessions directory", async (t) => {
  const configDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "community-kimi-escape-"));
  t.after(() => fs.rmSync(configDir, { recursive: true, force: true }));
  await fs.promises.writeFile(path.join(configDir, "session_index.jsonl"), `${JSON.stringify({
    sessionId: SESSION_ID,
