...
export default function Composer({ centered = false, quick = false }) {
  ...
  const slashCommands = [
    ...
    { id: "init", label: "Init", desc: "Create an AGENTS.md file with project instructions", icon: <IconCmdInit size={16} />,
      run: () => doSendRef.current?.("Create an AGENTS.md file with project instructions") },
    ...
    { id: "plan", label: "Plan mode", desc: useStore.getState().planMode ? "Turn plan mode off" : "Turn plan mode on", icon: <IconCmdPlan size={16} />,
      run: () => {
        ...
        s.toast(s.planMode ? "Plan mode off" : "Plan mode on — the assistant will propose a plan first");
      } },
